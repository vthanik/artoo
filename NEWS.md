...
* `apply_spec()` gained `on_lossy`: integer coercion that would truncate
  ...
  (`artoo_error_type`) before any value is damaged; `on_lossy = "warn"`
...
* Character ISO 8601 date/datetime/time columns (the SDTM `--DTC`
  ...
  column is `Date`/`POSIXct`/`artoo_time`), and ISO text is never silently
...
* New `conformance()` reads the findings `apply_spec()` attached, as a
  `artoo_findings` frame with a sectioned print; `check_spec()` returns the
...
* `read_spec()` gained `datasets` (scope a multi-domain spec to the
  ...
  `apply_spec()`. `artoo_spec()` itself now rejects duplicate
...
* `spec_variables()` and `spec_codelists()` now document every column of
  ...
  a "why artoo" comparison.
* `read_dataset()` now guarantees its contract for every codec: a truncated
  or bit-flipped file aborts with `artoo_error_codec` instead of leaking a
...
* Added fuzzing (`test-fuzz-codecs.R`: truncation at every boundary, header
  ...
  `ARTOO_STRESS=1`), an opt-in performance-regression check (`ARTOO_BENCH=1`
...
* New `sync_meta()` re-aligns a `artoo_meta` with a transformed frame
...
* `apply_spec()` decode matching gained `trim` (default `TRUE`: a value that
  ...
  `artoo_warning_codelist` naming the variants) and `ignore_case` (opt-in
...
* `check_spec()` gained five conformance dimensions, each with a
  `artoo_checks()` toggle: `variable_name` and `dataset_name` (XPORT naming
...
* SAS special missing tags (`.A`-`.Z`, `._`) now survive every format, not
  ...
  `_artoo.specialMissings` extension while the data values stay plain nulls
...
* `read_parquet()` now synthesizes metadata from the column types and
  attributes when a foreign (plain-nanoparquet/arrow) file carries no artoo
...
* `write_json()` gained `strict`: `TRUE` writes a pure closed-vocabulary
  CDISC file and warns (`artoo_warning_codec`) naming any dropped artoo
  extensions; the default (`FALSE`) emits a single namespaced `_artoo` block
...
* `read_xpt()` aborts (`artoo_error_codec`) on a multi-member transport file
...
* `read_xpt()` and the other readers project the column `label` and SAS
  ...
  labelled/gtsummary/viewer tooling sees them; the `artoo_meta` stays the
  source of truth.
* `write_xpt()` warns (`artoo_warning_encoding`) when a value would write a
...
* A failed atomic move now aborts (`artoo_error_codec`) instead of leaving the
  caller believing a file was written.
* The bundled `cdisc_*` demo data is now built with artoo's own `read_xpt()`
  (dogfooding). Blank character cells read back as `NA` (artoo's XPORT
...
* `artoo_checks()` dropped its `encoding_check` argument; the submission
...
* `artoo_checks()` gained the four matching toggles
...
* `as.data.frame()` on a `artoo_check` returns its findings data frame.
* `read_json()` and `write_json()` read and write CDISC Dataset-JSON v1.1
  files through the `artoo_meta` spine, with byte-stable output (injectable
...
* `read_parquet()` and `write_parquet()` read and write Apache Parquet files
  via the lightweight `nanoparquet` engine, embedding the full `artoo_meta`
...
* `write_parquet()` and `write_rds()` gained an `encoding` argument that
  records the data's source charset in the `artoo_meta` (the containers stay
...
* `apply_spec()` warns (class `artoo_warning_coercion`) when an `integer`
...
* `read_dataset()` and `read_rds()` refuse a payload that is not a data
  frame with a `artoo_error_codec` instead of returning it silently.
...
* `artoo_time` and `artoo_check` print/format methods now dispatch in
  installed builds; previously the data viewer and console showed raw
  seconds.
* `artoo_time` gained `[<-`/`[[<-` guards that reject a character right-hand
  side (`artoo_error_input`) instead of silently corrupting the column;
  `c()` now accepts a bare `NA`; `unique()` and `mean()` keep the class; and
  `%%`/`%/%` preserve it so a day-wrap (`t %% 86400`) stays a clock time.
* `artoo_time` comparison with a character value (`t == "08:30:00"`) now
  aborts (`artoo_error_input`) instead of silently coercing to a string
...
* `read_xpt()` and `write_xpt()` read and write SAS XPORT (xpt) v5 and v8
  files losslessly through the `artoo_meta` spine, with byte-stable output, a
  ...
  charset.
* `artoo_spec` and `artoo_meta` now print a readable summary (header, slot
...
* `apply_spec()` no longer silently abandons row ordering when only some
  ...
  trails the rest, warning with class `artoo_warning_order`.
* `apply_spec()` scaffold and drop progress messages now carry the
  condition class `artoo_message_apply`, so a pipeline can muffle them with
  `withCallingHandlers()` without suppressing other output.
* `read_spec()` informs (class `artoo_message_p21_sheet`) which sheet it
