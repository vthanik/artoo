...
the SAS `BASECHAR()` function exists for exactly this. artoo's equivalent is
...
`«»` → `<<` `>>`, `½` → `1/2`). The table is pinned inside artoo, so —
unlike `iconv //TRANSLIT`, whose output differs between Linux and macOS
— the fold is byte-identical on every platform:
