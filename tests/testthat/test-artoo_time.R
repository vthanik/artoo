# Tests for the artoo_time S3 class: a clock-time-of-day value (seconds
...
test_that("constructor coerces to double and tags the class", {
  t <- artoo_time(c(0, 3600, 86399))
  expect_true(is_artoo_time(t))
  expect_type(unclass(t), "double")
  expect_identical(unclass(t), c(0, 3600, 86399))
})

test_that("is_artoo_time is FALSE for other objects", {
  expect_false(is_artoo_time(1L))
  expect_false(is_artoo_time(Sys.Date()))
  expect_false(is_artoo_time("08:00:00"))
})

test_that("format renders HH:MM:SS, NA, and times past 24h", {
  t <- artoo_time(c(0, 30600, 50700, 86399, NA, 90000))
  expect_identical(
    format(t),
...
test_that("format renders negative (elapsed) times with a sign", {
  expect_identical(format(artoo_time(-3600)), "-01:00:00")
})

test_that("as.character matches format", {
  t <- artoo_time(c(0, 3661))
  expect_identical(as.character(t), c("00:00:00", "01:01:01"))
})

test_that("print is stable", {
  expect_snapshot(print(artoo_time(c(0, 30600, NA))))
})

test_that("subsetting preserves the class", {
  t <- artoo_time(c(10, 20, 30))
  expect_true(is_artoo_time(t[2:3]))
  expect_identical(unclass(t[2:3]), c(20, 30))
  expect_true(is_artoo_time(t[[1]]))
})
...
test_that("c and rep preserve the class", {
  ...
  expect_true(is_artoo_time(c(a, b)))
  expect_identical(unclass(rep(b, 3)), c(3, 3, 3))
  expect_true(is_artoo_time(rep(b, 3)))
})

test_that("c.artoo_time rejects an incompatible part with caller attribution", {
  a <- artoo_time(c(1, 2))
  expect_error(c(a, "x"), class = "artoo_error_input")
  # The error attributes to the c() call, not the internal lapply lambda.
  expect_snapshot(c(a, "x"), error = TRUE)
})

test_that("a artoo_time survives as a data.frame column", {
  df <- data.frame(
    id = 1:3,
    tm = artoo_time(c(0, 30600, 86399))
  )
  expect_true(is_artoo_time(df$tm))
  expect_identical(format(df$tm)[2], "08:30:00")
})

test_that("ordering and sorting work via xtfrm", {
  t <- artoo_time(c(30, 10, 20))
...
test_that("min/max/range return artoo_time via Summary", {
  t <- artoo_time(c(30, 10, 20))
  expect_true(is_artoo_time(min(t)))
...
test_that("arithmetic returns artoo_time; unary minus negates", {
  a <- artoo_time(100)
  b <- artoo_time(40)
  expect_true(is_artoo_time(a - b))
  expect_identical(unclass(a - b), 60)
  expect_identical(unclass(-a), -100)
})

test_that("sum returns the total seconds (not a clock time)", {
  expect_identical(sum(artoo_time(c(10, 20, 30))), 60)
})

# ---- Wave 2: assignment guards, combine, summary, Ops, fractional -----------

test_that("[<- with a numeric or artoo_time RHS keeps the class", {
  t <- artoo_time(c(10, 20, 30))
  t[2] <- 99
  expect_true(is_artoo_time(t))
...
test_that("c accepts a logical NA element", {
  out <- c(artoo_time(c(1, 2)), NA)
  expect_true(is_artoo_time(out))
  expect_identical(is.na(out), c(FALSE, FALSE, TRUE))
})

test_that("unique and mean keep the class", {
  t <- artoo_time(c(10, 10, 20))
  expect_true(is_artoo_time(unique(t)))
  expect_identical(unclass(unique(t)), c(10, 20))
  expect_true(is_artoo_time(mean(t)))
  expect_identical(unclass(mean(artoo_time(c(10, 20)))), 15)
})

test_that("modulo and integer-divide preserve the class (day wrap)", {
  t <- artoo_time(90000) # 25:00:00
  wrapped <- t %% 86400
  expect_true(is_artoo_time(wrapped))
  expect_identical(unclass(wrapped), 3600)
  expect_true(is_artoo_time(t %/% 2))
})

test_that("comparison with a character value aborts (review)", {
  t <- artoo_time(30600)
  expect_error(t == "08:30:00", class = "artoo_error_input")
  expect_error("08:30:00" == t, class = "artoo_error_input")
...
test_that("format renders fractional seconds (hms convention)", {
  expect_identical(format(artoo_time(30600.5)), "08:30:00.5")
  expect_identical(
    format(artoo_time(c(30600, 30600.25))),
    c(
      "08:30:00",
      "08:30:00.25"
    )
  )
  expect_identical(format(artoo_time(-3600.5)), "-01:00:00.5")
})
...
test_that("the namespace holds no local print/format bindings (review B1)", {
  ...
  # *.artoo_time method: registerS3methods then registers them into artoo's
...
test_that("format/print dispatch for artoo_time from outside the namespace (review B1)", {
  # In-namespace tests find format.artoo_time lexically even when registration
  # is broken; evaluate from an environment that does NOT chain into artoo.
  e <- new.env(parent = baseenv())
  e$t <- artoo_time(c(30600, NA))
  expect_identical(eval(quote(format(t)), e), c("08:30:00", NA))
  expect_output(eval(quote(print(t)), e), "<artoo_time\\[2\\]>")
})
